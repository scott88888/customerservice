<?php

   /**
    *
    * Editor.md PHP简易上傳類（仅供演示用，生产环境使用还需谨慎考虑）
    *
    * @FileName: editormd.uploader.class.php
    * @Auther: Pandao
    * @E-mail: pandao@vip.qq.com
    * @CreateTime: 2015-02-13 23:31:32
    * @UpdateTime: 2015-06-02 15:36:23
    * Copyright@2015 Editor.md all right reserved.
    */

    class EditorMdUploader
    {
        public $files;                          // $_FILES數组
        public $fileExit;                       // 文件扩展名
        public $saveName;                       // 最终保存的文件名
        public $saveURL;                        // 最终保存URL地址
        public $savePath;                       // 保存本地文件路徑
        public $randomLength   = '';            // 產生随機文件名的長度，当為日期时為日期的格式
        public $randomNameType = 1;             // 產生随機的形式, NULL為保留原文件名, 1產生随機字符串, 2產生日期文件名
        public $formats = array(                // 允许上傳的文件格式
            'gif', 'jpg', 'jpeg', 'png', 'bmp'
        );
        public $maxSize        = 1024;          // 最大上傳文件大小，單位KB
        public $cover          = true;          // 是否覆盖同名文件, 1覆盖,0不覆盖
        public $redirect       = false;         // 是否进行URL跳转
        public $redirectURL    = "";            // 上傳成功或出错後要转到的URL        
        public $errors = array(                 // 錯誤訊息
            'empty'      => '上傳文件不能為空',
            'format'     =>'上傳的文件格式不符合规定',
            'maxsize'    => '上傳的文件太大',
            'unwritable' => '保存目錄不可寫，請更改权限',
            'not_exist'  => '保存目錄不存在',
            'same_file'  => '已经有相同的文件存在'
        );

        /**
         * 构造函數，初始化對象
         *
         * @access  public
         * @param   string          $savePath       最终保存的本地路徑
         * @param   string          $saveURL        最终保存的URL地址
         * @param   string          $formats        允许上傳的文件格式
         * @param   Number          $randomNameType 是否產生随機文件名及形式
         * @param   Intiger/string  $randomLength   產生随機文件名字符的長度
         * @param   boolean         $cover          是否覆盖相同文件
         * @param   Intiger         $maxSize        允许最大的上傳文件大小，以KB為單位
         * @return  viod
         */
        
        public function __construct($savePath, $saveURL, $formats, $randomNameType = 1, $randomLength = '', $cover = true, $maxSize = 1024)
        {
            $this->savePath       = $savePath;
            $this->saveURL        = $saveURL;
            $this->formats        = $formats;
            $this->maxSize        = $maxSize;
            $this->cover          = $cover;
            $this->randomNameType = $randomNameType;
            $this->randomLength   = $randomLength;
        }

        /**
         * 配置参數函數
         *
         * @access  public
         * @param   array           $configs        配置项數组
         * @return  void
         */
        
        public function config($configs) 
        {
            foreach($configs as $key => $value)
            {
                $this->$key = $value;
            }       
        }

        /**
         * 執行文件上傳
         *
         * @access  public
         * @param   string          $name           fileInput's name
         * @return  boolean                         返回是否上傳成功的布尔值
         */

        public function upload($name)
        {            
            if(empty($_FILES[$name]['name'])) //上傳文件為空时
            {
               $this->message($this->errors['empty']);
                
                return false;
            }
            
            $this->files = $_FILES[$name];
            
            if(!file_exists($this->savePath)) //目錄不存在
            {
                $this->message($this->errors['not_exist']);
                
                return false;
            }

            if(!is_writable($this->savePath)) //目錄不可寫
            {
                $this->message($this->errors['unwritable']);
                
                return false;
            }

            $this->fileExt  = $this->getFileExt($this->files["name"]); //取得扩展名
            
            $this->setSeveName();
            
            return $this->moveFile();
        }

        /**
         * 檢查並移動上傳文件
         *
         * @access  private
         * @return  boolean
         */

        private function moveFile()
        {
            $files = $this->files;
            
            if ($this->formats != "" && !in_array($this->fileExt, $this->formats))
            {
                $formats = implode(',', $this->formats);
                $message = "您上傳的文件" . $files["name"] . "是" . $this->fileExt . "格式的，系统不允许上傳，您只能上傳" . $formats . "格式的文件。";
                $this->message($message);
                
                return false;
            }

            if ($files["size"] / 1024 > $this->maxSize)
            {
                $message = "您上傳的 " . $files["name"] . "，文件大小超出了系统限定值" . $this->maxSize . " KB，不能上傳。";
                $this->message($message);
                
                return false;
            }

            if (!$this->cover) //当不能覆盖时
            {
                if(file_exists($this->savePath.$this->saveName)) //有相同的文件存在
                {
                    $this->message($this->saveName . $this->errors['same_file']);
                    
                    return false;
                }
            }

            if (!@move_uploaded_file($files["tmp_name"], iconv("utf-8", "gbk", $this->savePath . $this->saveName)))
            {                
                switch($files["errors"])
                {
                    case '0':
                        $message = "文件上傳成功";
                        break;
                    
                    case '1':
                        $message = "上傳的文件超过了 php.ini 中 upload_max_filesize 选项限制的值";
                        break;
                    
                    case '2':
                        $message = "上傳文件的大小超过了 HTML 表單中 MAX_FILE_SIZE 选项指定的值";
                        break;
                    
                    case '3':
                        $message = "文件只有部分被上傳";
                        break;
                    
                    case '4':
                        $message = "没有文件被上傳";
                        break;
                    
                    case '6':
                        $message = "找不到临时目錄";
                        break;
                    
                    case '7':
                        $message = "寫文件到硬盘时出错";
                        break;
                    
                    case '8':
                        $message = "某个扩展停止了文件的上傳";
                        break;
                    
                    case '999':
                    default:
                        $message = "未知錯誤，請檢查文件是否损坏、是否超大等原因。";
                        break;
                }

                $this->message($message);
                
                return false;
            }
            
            @unlink($files["tmp_name"]); //刪除临时文件
            
            return true;
        }

        /**
         * 產生随機文件名函數
         *
         * @access  private
         * @return  string         $fileName      返回產生的文件名字符串
         */

        private function randomFileName()
        {
            if ($this->randomNameType == 1)        // 產生時間格式文件名
            {
                date_default_timezone_set('PRC');  //設定时区
                
                $date     = date($this->randomLength);
                $fileName = $date . "_" . mt_rand(10000, 99999);
            }
            elseif ($this->randomNameType == 2)    // 產生随機字符文件名
            {
                $chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz';
                $max   = strlen($chars) - 1;
                mt_srand((double)microtime() * 1000000);
                
                for($i = 0; $i < $this->randomLength; $i++)
                {
                    $fileName .= $chars[mt_rand(0, $max)];
                }
            }
            else
            {
                $fileName = '';
            }

            return $fileName;
        }

        /**
         * 组成最终保存的完整路徑及文件名
         *
         * @access  private
         * @return  void
         */
        
         private function setSeveName()
         {             
            $this->saveName = $this->randomFileName().".".$this->fileExt;
             
            if($this->saveName == '') //如果没產生随機文件名，就保留原文件名
            {
                $this->saveName = $this->files['name'];
            }
         }

        /**
         * 取得保存的文件名，用于資料庫存储
         *
         * @access  public
         * @return  string
         */
        
         public function getSeveName()
         {
             return $this->saveName;
         }

        /**
         * 取得文件後缀名函數
         *
         * @access  public
         * @return  string
         */
        
         public function getFileExt($fileName)
         {
             return trim(strtolower(substr(strrchr($fileName, '.'), 1)));
         }

        /**
         * 上傳成功或出错後跳转
         *
         * @access  public
         * @param   array           $configs        配置项數组
         * @return  void
         */
        
        public function redirect() 
        {            
            header('location: ' . $this->redirectURL);
        }

        /**
         * 錯誤提示函數
         *
         * @access  public
         * @return  void
         */

        public function message($message, $success = 0)
        {
            $array = array(
                'success' => $success
            );
            
            $url = $this->saveURL . $this->saveName;

            // 适用于跨域上傳时，跳转到中介頁面等
            if ($this->redirect) 
            {
                $this->redirectURL .= "&success=" . $success . "&message=" . $message;
                
                if ($success == 1)
                {
                    $this->redirectURL .= '&url=' . $url;
                }
                
                $this->redirect();
            }
            else
            {
                if ($success == 1)
                {
                    $array['url']    = $url;
                }
                else
                {
                    $array['message'] = $message; 
                }
                
                echo json_encode($array);
            }
        }
    }
