...
(function () {
    ...
    window.UEDITOR_CONFIG = {
        ...
        //,zIndex : 900     //編輯器层级的基數,默认是900

        //若实例化編輯器的頁面手動修改的domain，此处需要設定為true
        //,customDomain:false
        ...
        //,imagePopup:true      //图片操作的浮层開关，默认打開

        //,autoSyncData:true //自動同步編輯器要送出的資料
        //,emotionLocalization:false //是否開啟表情本地化，默认關閉。若要開啟請确保emotion文件夹下包含官網提供的images表情文件夹
        ...
        //insertorderedlist
        //有序列表的下拉配置,值留空时支持多語言自動识别，若配置值，则以此值為准
        //,'insertorderedlist':{
        //      //自定的樣式
        //        'num':'1,2,3...',
        //        'num1':'1),2),3)...',
        ...
        //insertunorderedlist
        //無序列表的下拉配置，值留空时支持多語言自動识别，若配置值，则以此值為准
        ...
        //    'square' : ''   //'■ 小方块'
        //}
        //,listDefaultPaddingLeft : '30'//默认的左边缩进的基數倍
        //,listiconpath : 'http://bs.baidu.com/listicon/'//自訂标号的路径
        //,maxListLevel : 3 //限制可以tab的级數, 設定-1為不限制

        //,autoTransWordToList:false  //禁止word中粘贴进来的列表自動变成列表标签

        //fontfamily
        //字体設定 label留空支持多語言自動切换，若配置，则以配置值為准
        //,'fontfamily':[
        ...
        //paragraph
        //段落格式 值留空时支持多語言自動识别，若配置，则以配置值為准
        ...
        //customstyle
        //自訂樣式，不支持国际化，此处配置值即可最后显示值
        //block的元素是依据設定段落的逻辑設定的，inline的元素依据BIU的逻辑設定
        //尽量使用一些常用的标签
        //参數说明
        //tag 使用的标签名字
        //label 显示的名字也是用来標識不同类型的標識符，注意这个值每个要不同，
        //style 新增的樣式
        //每一个對象就是一个自訂的樣式
        //,'customstyle':[
        ...
        //    {
        //        label:'',       //显示的名稱
        //        cmdName:'selectall',//执行的command命令，当點擊这个右键選單时
        //        //exec可选，有了exec就会在點擊时执行这个function，優先级高于cmdName
        //        exec:function () {
        //            //this是当前編輯器的实例
       ...
       ,elementPathEnabled : false
        ...
        //字數统计提示，{#count}代表当前字數，{#leave}代表还可以输入多少字符數,留空支持多語言自動切换，否则按此配置显示
        //,wordCountMsg:''   //当前已输入 {#count} 个字符，您还可以输入{#leave} 个字符
        //超出字數限制提示  留空支持多語言自動切换，否则按此配置显示
        ...
        //scaleEnabled
        //是否可以拉伸長高,默认true(当開啟时，自動長高失效)
        //,scaleEnabled:false
        //,minFrameWidth:800    //編輯器拖動时最小宽度,默认800
        //,minFrameHeight:220  //編輯器拖動时最小高度,默认220


        //,topOffset:30
        //編輯器底部距离工具栏高度(如果参數大于等于編輯器高度，则設定無效)
        //,toolbarTopOffset:400

        //設定远程图片是否抓取到本地保存
        //,catchRemoteImageEnable: true //設定是否抓取远程图片
        ...
        //    imageBlockLine: 'center',       //图片的浮動方式，独占一行剧中,左右浮動，默认: center,left,right,none 去掉这个属性表示不执行排版
        //    pasteFilter: false,             //根據规则过滤没事粘贴进来的内容
        ...
        //sourceEditor
        //源碼的查看方式,codemirror 是程式碼高亮，textarea是文本框,默认是codemirror
        ...
        //編輯器初始化完成后是否进入源碼模式，默认為否。
        //,sourceEditorFirst:false

        //iframeUrlMap
        //dialog内容的路径 ～会被替换成URL,垓属性一旦打開，将覆盖所有的dialog的默认路径
        //,iframeUrlMap:{
        ...
        //webAppKey 百度应用的APIkey，每个站長必须首先去百度官網注册一个key后方能正常使用app功能，注册介绍，http://app.baidu.com/static/cms/getapikey.html
        //, webAppKey: ""
