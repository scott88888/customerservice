...
class Route
{
     ...
     * @param array        $option  路由参數
     * @param array        $pattern 变量規則
     * @return void
     */
     ...
     * @param string $type 绑定類型 默認為module 支援 namespace class controller
     * @return mixed
     */
     ...
     * 設定或者取得路由標識
     * @access public
     * @param string|array $name  路由命名標識 數组表示批量設定
     ...
     * @param array  $option  分組路由参數
     * @param array  $pattern 分組变量規則
     * @return void
     */
     ...
     * 註冊路由分組
     * @access public
     * @param string|array   $name    分組名稱或者参數
     ...
     * 取得或者批量設定路由定義
     * @access public
     * @param mixed $rules 請求類型或者路由定義數组
     * @return array
     */
    ...
    public static function checkDomain($request, &$currentRules, $method = 'get')
    {
        ...
        if (!empty($rules)) {
            ...
            if (isset($rules[$host])) {
                ...
                if ($rootDomain) {
                    // 配置域名根 例如 thinkphp.cn 163.com.cn 如果是国家级域名 com.cn net.cn 之類的域名需要配置
     ...
     * @param bool    $checkDomain 是否检测域名規則
     * @return false|array
     */
     ...
     * @access public
     * @param string $url        URL地址
     * @param string $controller 控制器名 （支援带模組名 index/user ）
     * @param string $depr       URL分隔符
     * @return array
     */
     ...
     * @access public
     * @param string $url        URL地址
     * @param string $controller 控制器類名（带命名空間）
     * @param string $depr       URL分隔符
     * @return array
     */
    ...
    private static function checkOption($option, $request)
    {
        if ((isset($option['method']) && is_string($option['method']) && false === stripos($option['method'], $request->method()))
            || (isset($option['ajax']) && $option['ajax'] && !$request->isAjax()) // Ajax检测
             ...
             || (isset($option['ext']) && false === stripos('|' . $option['ext'] . '|', '|' . $request->ext() . '|')) // 伪静态後缀检测
             || (isset($option['deny_ext']) && false !== stripos('|' . $option['deny_ext'] . '|', '|' . $request->ext() . '|'))
            || (isset($option['domain']) && !in_array($option['domain'], [$_SERVER['HTTP_HOST'], self::$subDomain])) // 域名检测
             || (isset($option['https']) && $option['https'] && !$request->isSsl()) // https检测
             || (isset($option['https']) && !$option['https'] && $request->isSsl()) // https检测
             || (!empty($option['before_behavior']) && false === Hook::exec($option['before_behavior'])) // 行為检测
             || (!empty($option['callback']) && is_callable($option['callback']) && false === call_user_func($option['callback'])) // 自訂检测
        ) {
            return false;
        }
        return true;
    ...
    /**
     * 解析模組的URL地址 [模組/控制器/操作?]参數1=值1&参數2=值2...
     * @access public
     * @param string $url        URL地址
     * @param string $depr       URL分隔符
     ...
     * @access private
     * @param string $url     URL地址
     * @param bool   $convert 是否自動轉換URL地址
     * @return array
     */
    ...
    /**
     * 解析URL地址中的参數Request對象
     * @access private
     * @param string $url 路由規則
     * @param array  $var 变量
     * @return void
     */
