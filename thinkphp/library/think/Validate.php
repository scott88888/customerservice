...
class Validate
{
     ...
     * 新增字段驗證規則
     * @access protected
     * @param string|array  $name  字段名稱或者規則數组
     ...
     * 設定驗證規則的默認提示訊息
     * @access protected
     * @param string|array  $type  驗證規則類型名稱或者數组
     ...
     * 設定驗證场景
     * @access public
     * @param string|array  $name  场景名或者场景設定數组
    ...
    {
        // 支援多規則驗證 require|in:a,b,c|... 或者 ['require','in'=>'a,b,c',...]
        if (is_string($rules)) {
            $rules = explode('|', $rules);
        }
        $i = 0;
    ...
    /**
     * 驗證是否為合格的域名或者IP 支援A，MX，NS，SOA，PTR，CNAME，AAAA，A6， SRV，NAPTR，TXT 或者 ANY類型
     * @access protected
     * @param mixed     $value  字段值
     * @param mixed     $rule  驗證規則
     * @return bool
     ...
     * @access protected
     * @param mixed     $value  字段值
     * @param mixed     $rule  驗證規則 格式：資料表,字段名,排除ID,主键名
     * @param array     $data  資料
     ...
     * @access protected
     * @param mixed     $value  字段值
     * @param mixed     $rule  驗證規則 正則規則或者预定義正則名
     * @return mixed
     */
    ...
    protected function token($value, $rule, $data)
    {
        ...
        if (isset($data[$rule]) && Session::get($rule) === $data[$rule]) {
            // 防止重复送出
            Session::delete($rule); // 驗證完成销毁session
            return true;
        }
        // 開啟TOKEN重置
        Session::delete($rule);
